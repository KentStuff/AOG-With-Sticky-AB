            ...
            if (ptCount > 0)
            {
                ...
                else
                {
                    ...
                    steerAngleYT = glm.toDegrees(Math.Atan(2 * (((goalPointYT.easting - pivot.easting) * Math.Cos(localHeading))
                        + ((goalPointYT.northing - pivot.northing) * Math.Sin(localHeading))) * (mf.vehicle.wheelbase+(mf.vehicle.trackWidth/2)) / goalPointDistanceSquared));
                    ...
                    double angVel = 0.277777 * mf.pn.speed * (Math.Tan(glm.toRadians(steerAngleYT))) / mf.vehicle.wheelbase;

                    //clamp the steering angle to not exceed safe angular velocity
                    if (Math.Abs(angVel) > mf.vehicle.maxAngularVelocity)
                    {
                        steerAngleYT = glm.toDegrees(Math.Atan(mf.vehicle.wheelbase * (steerAngleYT > 0 ? mf.vehicle.maxAngularVelocity : -mf.vehicle.maxAngularVelocity) / (mf.avgSpeed * 0.277777)));
                    }
