...
namespace AgOpenGPS
{
    public class CABCurve
    {
        ...
        public void BuildCurveCurrentList(vec3 pivot)
        {
            ...
            vec3 ABLOOK = new vec3(pivot.easting + (Math.Sin(mf.fixHeading) * 5), pivot.northing + (Math.Cos(mf.fixHeading) * 5), 0);
        ...
        public void GetCurrentCurveLine(vec3 pivot, vec3 steer)
        {
            ...
            else
            {
                ...
                //if ((Math.Abs(lastCurveDistance) > (0.1 * mf.tool.toolWidth)) || !isCurveValid)
            ...
            if (ptCount > 0)
            {
                ...
                else    // Pure Pursuit ------------------------------------------
                {
                    ...
                    steerAngleCu = glm.toDegrees(Math.Atan(2 * (((goalPointCu.easting - pivot.easting) * Math.Cos(localHeading))
                        + ((goalPointCu.northing - pivot.northing) * Math.Sin(localHeading))) * (mf.vehicle.wheelbase+(mf.vehicle.trackWidth/2)) / goalPointDistanceSquared));
                    ...
                    double angVel = 0.277777 * mf.pn.speed * (Math.Tan(glm.toRadians(steerAngleCu))) / mf.vehicle.wheelbase;

                    //clamp the steering angle to not exceed safe angular velocity
                    if (Math.Abs(angVel) > mf.vehicle.maxAngularVelocity)
                    {
                        steerAngleCu = glm.toDegrees(steerAngleCu > 0 ?
                                (Math.Atan((mf.vehicle.wheelbase * mf.vehicle.maxAngularVelocity) / (mf.avgSpeed * 0.277777)))
                            : (Math.Atan((mf.vehicle.wheelbase * -mf.vehicle.maxAngularVelocity) / (mf.avgSpeed * 0.277777))));
                    }
